...
const Popup = ({ isOpen, cb, className, children }: PopupProps) => {
  ...
  return (
    <div
      ref={popupRef}
      className={cn(
        'absolute z-10 mt-1 hidden w-max max-w-sm flex-col rounded border border-border bg-zinc-50 p-2 text-white shadow-lg shadow-zinc-400 dark:border-border-dark dark:bg-zinc-950 dark:shadow-zinc-950',
        { flex: isOpen },
        className
      )}
    >
